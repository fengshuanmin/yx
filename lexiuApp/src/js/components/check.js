/**
 * Created by Administrator on 2016/7/23 0023. APP查勘
 */
	...
	componentDidMount: function() {
		  ...
		  for (var i = 0; i < sss.length; i++) {
			  ...
			  sss.eq(i).bind("click",function(){
					 ...
					 for(var j = 0;j < data.length; j++){
						  if(inde == j){
								var idata = data[j];
								a.props.history.pushState(idata, "/check_detail");
						  }
					 }
			  })
		  }
      ...
      render() {
			  ...
			  for (var i = 0; i < arr.length; i++) {
				  ...
				  var progress = "";
				  if (item_survey.loss_state == 0 || item_survey.loss_state == "" || item_survey.loss_state == null) {
					  progress = "需估损案件";
				  } else {
					  progress = "";
				  }
				  ...
				  item_checkNodes.push(
						  <div className="listContainer clearfix" style={i%2==0?odd:eve} key={item_survey.id}>
							  ...
							  <li><span>车型 :</span>{item_survey.cximc}</li>
    	...
    	return (
    		  <div className="item_survey_list clearfix">
	    		  <div className="headerInfo">
					<HeaderIf numBer={this.state.rectotal} name="查勘"></HeaderIf>
