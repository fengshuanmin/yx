...
export default class Info extends React.Component {
    constructor(props) {
                        ...
                        success:(msg)=>{
                            if(msg.errorCode=='000000'){
                               /* const { List = [] } = this.state;
                                List.map((il,ix)=>{
                                    il.enquiryPatrs && il.enquiryPatrs.map((le,ie)=>{
                                        if(le.PartId==partid){
                                            il.enquiryPatrs.splice(ie,1)
                                        }
                                    })
                                })
                                this.setState({
        ...
        this.submit=()=>{
            ...
            console.log(have)
            console.log(none)
            /*for(var i in this.state.zhuangtaiList){
                if(this.state.zhuangtaiList[i].PartType!=''&&this.state.zhuangtaiList[i].isPurchase!=1){
                    have.push(this.state.zhuangtaiList[i])
                }else if(this.state.zhuangtaiList[i].PartType==''&&this.state.zhuangtaiList[i].isPurchase!=1){
                    none.push(this.state.zhuangtaiList[i])
                }
            }*/
            ...
            <div>
                {
                    this.state.loading ?
                        <div className="infotop">
                            ...
                            {this.state.caigouList.length==0?<div style={{background:'#fff',marginTop:'0.2rem',overflow:'hidden',marginBottom:'1.2rem'}}>
                                <this.props.BaseLi data={[{
                                    Style:{ico2:'&#xe607;',S:{color:'#97989b'}},
                                    key:'管理费测算或下载估损单',
                                    fun:this.check
                                }]} {...this.props}/>
                            </div>:<div style={{background:'#fff',marginTop:'0.2rem',overflow:'hidden'}}>
                                <this.props.BaseLi data={[{
                                    Style:{ico2:'&#xe607;',S:{color:'#97989b'}},
                                    key:'管理费测算或下载估损单',
                                    fun:this.check
                            ...
                            {
                                this.state.caigouList.length==0?<div></div>:
                                    <div style={{background:'#fff',marginTop:'0.2rem',marginBottom:'1.2rem'}}>
                                        <this.props.InfoTitle T={this} data={{
                                            key:'采购订单',Lcolor:'red'}}/>
                                        <ul>
                                            {this.state.caigouList==null?<li style={{lineHeight:'1.2rem',textAlign:'center'}}>暂无采购订单</li>:
                                                this.state.caigouList.map((item,index)=>{
                                                    return(
                                                        <li key={index} style={{marginLeft:'0.2rem',lineHeight:'0.8rem',borderBottom:'0.01rem solid #ccc'}} onClick={()=>{this.props.history.pushState({id:item.id,orderStatus:item.orderStatus,taskId:item.taskId,flag:'purchaseinfo'},'/dingdan/dingdan')}}><span>{item.code}</span>
                                                            <span className="iconfonts" style={{display:'inline-block',float:'right',marginRight:'0.4rem'}}>&#xe607;</span>
                                                            <span style={{display:'inline-block',float:'right',color:'#f66'}}>
                                                {item.orderStatus=='100'?'待付款':''||item.orderStatus=='101'?'待发货':''||item.orderStatus=='102'?'已发货':''||item.orderStatus=='103'?'已收货':''||item.orderStatus=='105'?'支付失败':''||item.orderStatus=='200'?'已取消':''}
                                                </span>
                                                        </li>
                                                    )
                                                })
                ...
                <div style={{width:'100%',padding:'0.2rem 0',borderTop:'0.01rem solid #ddd',position:'fixed',bottom:'0',background:'#fff'}}>
                    <p style={{float:'right',marginRight:'0.3rem'}}>
                        {this.state.task.status==1&&<span style={{float:'left',padding:'0.1rem 0.3rem',borderRadius:'0.3rem',
                            border:'1px solid #ccc',marginRight:'0.25rem'}} onClick={this.cd}>催 单</span>}
                        {this.state.task.status==3&&<span style={{float:'left',padding:'0.1rem 0.3rem',border:'1px solid #ccc',
                            borderRadius:'0.3rem',marginRight:'0.25rem'}} onClick={this.cd}>催 单</span>}
                        <span style={{float:'left',padding:'0.1rem 0.3rem',border:'1px solid #ccc',borderRadius:'0.3rem',marginRight:'0.25rem'}} onClick={this.tjlj}>添加零件</span>
                        {this.state.task.status !=0 &&this.state.task.status !=1&&this.state.task.status !=4 && this.state.List && this.state.List.length!=0 &&
                        <span style={{float:'left',padding:'0.1rem 0.3rem',border:'1px solid #ccc',borderRadius:'0.3rem',marginRight:'0.25rem'}}
                            onClick={this.bjxx}>查看报价</span>}
                        {(this.state.task.status==2||this.state.task.status==3||this.state.task.status==5)&&(()=>zhuangtai.filter((item,index)=>item !=1))().length?
                            <span onClick={this.submit} style={{float:'left',padding:'0.1rem 0.3rem',color:'#f63e3e',border:'1px solid #f63e3e',
                                borderRadius:'0.3rem'}}>发起采购</span>:''}
                    </p>
                    {/*<ul style={{float:'right',marginRight:'0.3rem'}}>
                        {this.state.task.status==1&&<li style={{float:'left',padding:'0.1rem 0.3rem',border:'0.01rem solid #ccc',
                            borderRadius:'0.3rem',marginRight:'0.25rem'}} onClick={this.cd}>催 单</li>}
                        {this.state.task.status==3&&<li style={{float:'left',padding:'0.1rem 0.3rem',border:'0.01rem solid #ccc',
                            borderRadius:'0.3rem',marginRight:'0.25rem'}} onClick={this.cd}>催 单</li>}
                        <li style={{float:'left',padding:'0.1rem 0.3rem',border:'0.01rem solid #ccc',borderRadius:'0.3rem',marginRight:'0.25rem'}} onClick={this.tjlj}>添加零件</li>
                        {this.state.task.status !=0 &&this.state.task.status !=1&&this.state.task.status !=4 && this.state.List && this.state.List.length!=0 &&
                        <li style={{float:'left',padding:'0.1rem 0.3rem',border:'0.01rem solid #ccc',borderRadius:'0.3rem',marginRight:'0.25rem'}}
                            onClick={this.bjxx}>查看报价</li>}
                        {(this.state.task.status==2||this.state.task.status==3||this.state.task.status==5)&&(()=>zhuangtai.filter((item,index)=>item !=1))().length?
                            <li onClick={this.submit} style={{float:'left',padding:'0.1rem 0.3rem',color:'#f63e3e',border:'0.01rem solid #f63e3e',
                                borderRadius:'0.3rem'}}>发起采购</li>:''}
                    </ul>*/}
                </div>
            </div>
        )
