...
export default class Home extends React.Component{
    ...
    componentWillMount(){
        this.props.ajax({
            ...
            data:{token:this.props.user.data.token,showChannel:'3',showRoleType:'1'},
            suc:(data)=>{
                console.log(data)
                if(data.code=='0'){
                    this.setState({
                    ...
                    <div className="loginBox ln">
                        ...
                        <ul style={{fontSize:'0.32rem'}}>
                            <li>
                                ...
                                <input value={this.state.phoneval} ref="phone" disabled placeholder="请输入手机号" onBlur={this.blur} type="text" name="phone"/>
                        ...
                        <div className="DSYHome">
                            <ul>
                                <li>
                                    <span className="img"><img src={this.state.myJF && this.state.myJF.headImgUrl|| require("../../../img/tx.png")} alt=""/></span>
                                    <span>
                                <h4>{this.props.project.user.data.LxAqYhxxb.username}</h4>
                                        {/*<p>
                                    <span>当前积分:<span style={{color:"#fa4b4c",fontWeight:600}}>{this.state.myJF && this.state.myJF.currIntegralSum}</span></span>
                                    <span>总积分:<span style={{fontWeight:600}}>{this.state.myJF && this.state.myJF.totalGetSum}</span></span>
                                </p>*/}
                            </span>
                                </li>
                            </ul>
                            {zw.length>1||(zw.length==1&&zw[0]=='ATUTOEVAL')?<div style={{background:'#fff',marginTop:'0.2rem'}}>
                                <ul style={{display:'flex',padding:'0.2rem 0.3rem',borderBottom:' 0.01rem solid #f5f5f5'}}>
                                    <li style={{flex:'1',fontSize:'0.3rem',padding:'0',fontWeight:'600'}}>我的订单</li>
                                    <li style={{width:'1.2rem',padding:'0',fontSize:'0.28rem',color:'#999'}} onClick={()=>this.props.history.pushState({a:true,where:''},'/dingdan/list')}>全部订单</li>
                                    <li className="iconfonts" style={{width:'0.3rem',padding:'0',color:'#999'}} onClick={()=>this.props.history.pushState({a:true,where:''},'/dingdan/list')}>&#xe607;</li>
                                </ul>
                            ...
                            <span className="rightTopBorder">
                                积分榜<span className="iconfonts" style={{float:'right'}}>&#xe607;</span>
                            </span>
                        ...
                        <li onClick={()=>{this.props.history.pushState(null, "/IntegralRecord");}}>
                            <span className="iconfonts imgIcon"></span>
                            <span className="rightTopBorder">
                                积分记录<span className="iconfonts" style={{float:'right'}}>&#xe607;</span>
                            </span>

                        </li>*/}
                                {/*<li onClick={()=>{this.props.history.pushState(null, "/phonecode");}}>*/}
                                <li onClick={this.phonecodeevent}>
                                    <span className="iconfonts imgIcon"></span>
                                    <span className="rightTopBorder">
                                我的积分<span className="iconfonts" style={{float:'right'}}>&#xe607;</span>
                            </span>
                                ...
                                {/*<li onClick={()=>{this.props.history.pushState(null, "/message");}}>*/}
                                {/*<span className="iconfonts imgIcon"></span>*/}
                                {/*<span className="rightTopBorder">消息中心</span>*/}
                                {/*</li>*/}
                            </ul>
                        ...
                        <DSYButton ButtonFrom="DSYs" T={this} on={'home'} {...this.props}/>
                    </div>}
            </div>
        )
    }
