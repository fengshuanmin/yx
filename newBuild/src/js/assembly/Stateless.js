...
当未回收的时候判断==‘未派单’，如果==，则是未派单，！=是未回收
                */}
                ...
                </ul>
                {
                    ...
                    <p style={{display:'flex',textAlign:'left',padding:'3vw 4vw',borderTop:'0.01rem solid #F4F4F4'}}>
                        <time style={{color:'#999',flex:'1'}} dateTime={props.iteDat.pushTime || props.iteDat.createTime
                        || props.iteDat.time}>{props.timeString(props.iteDat.pushTime || props.iteDat.createTime ||
                            props.iteDat.time,'y-m-d h:m:s')}</time>
            ...
            return(
                <ul className="SPUlLeft" onTouchEnd={this.touchs.bind(this,'end')}
                    ...
                    {this.state.classification.map((item,index)=>{
                        return(
                            <li key={index} style={(item.childList && !this.state.showAddMy) ? {background:'#fff'}:{}} onClick={(e)=>{this.Ajaxc(item.id)}}>{item.name}</li>
                        )
                    })}
    ...
    DHRight:class extends React.Component {
        constructor(props) {
            ...
            this.AddOrMinus=(id,type,e)=>{
                ...
                // this.props.promptInfo({
                //     content:'请输入自定义零件名称！',Prompt:true
                // });
                                    ...
                                    <div key={index}>
                                        ...
                                        <ul >
                                        {
                                            ListItems.childList.map((item, index1) => {
                                                ...
                                                console.log(this.state.showWho)
                                            return (
                                                <li className="boxonce" key={index1}>
                                                    ...
                                                    {/*<div className="titles" style={{border:'0px',paddingLeft:'0.3rem',height:'1rem'}} onClick={this.showUl.bind(this,item.childList.length,index+'k'+index1)}>
                                                        {item.name && <span className="tvalue">{item.name}</span>}
                                                    </div>*/}
                                                    <ul className="UlboxTwo" style={this.state.showWho==index+'k'+index1 || ListItems.type=='addMy' ? {height:item.childList.length*0.9+'rem'}:{height:item.childList.length*0.9+'rem'}}>
                                                        {item.childList.map((mt, index2) => {
                                                            return (
                                                                <this.props.StateLess.AddAndC item={mt} AlreadyChosen={this.props.T.state.AlreadyChosen} key={index2}  T={this.props.T}/>
                                                            )
                                                        })}
                                                    </ul>
                                                </li>
    ...
    checkbox:class extends React.Component{
        ...
        render() {
            let img={
                checked:this.props.checked ? {background:'url('+require('../../img/fangxing.png')+')',backgroundSize:'100% 100%'}:{background:'url('+require('../../img/fangxingwei.png')+')',backgroundSize:'100% 100% '},
                disabled:this.props.disabled ? {background:'url('+require('../../img/cantChick@3x.png')+')',backgroundSize:'100% 100%'} :{}
            }
            return (
